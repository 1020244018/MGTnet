...
def get_1_ring(adjacency):
    
    for adj_sub in adjacency:
        sub_len = len(adj_sub)
        if sub_len !=10:
            for i in range(10-sub_len):
                adj_sub.append(0)
...
def get_adjs(Adj):
    # Adj: Adjacency matrix for each of the meshes
    res = []
    for adj in Adj:
        adjacency = []
        for index, a in enumerate(adj):
            col = list(a.nonzero()[1] + 1)
            # col.insert(0,index+1) #add self in adj
            adjacency.append(col)
        k = 0
        ...
        for adj_sub in adjacency:
            sub_len = len(adj_sub)
            if sub_len != k:
                for i in range(k-sub_len):
                    adj_sub.append(0)
        res.append(np.array(adjacency))
    return res
